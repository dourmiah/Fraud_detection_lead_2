...
```
The `create_fresh_project.ps1` utility script :
...
* You pass `parameters` to the trainig code `train.py`. See the end of the MLproject file which is currently commented. `train.py` will need to import `argparse`
...
* A la fin de ce tuto
* Copier les données sur le S3 de fraud2
* Modifier le code pour aller chercher les données à partir de là
* En profiter pour aller chercher un hypothétique jeux de données validées complémentaire
* Relancer le programme de test
